...
2. 新建 iOS 项目时使用 `./ios new`（或无参数运行 `./ios` 后选择 `new`），不要手工拼装 `.xcodeproj`。
...
7. 执行 Git 提交或代码 review 前，必须阅读 `checklists/pre-commit-review.md` 并运行 `./ios checklist . --purpose commit` 或 `--purpose review`；未通过时不得继续提交或给出 review 结论。
