...
2. 修改后运行最小相关验证。证据以 diff、配置和依赖锁定状态为键；键未变化则复用，变化才重跑。生成器变更在 `iOS_Workflow/` 内运行 `python3 -m unittest discover -s tests -v`。
...
- 新项目：`standards/project-generation.md`、`config/defaults.jsonc`、`standards/code-core.md`、所选语言规范、`standards/code-generation.md`、`standards/ui-style.md`、`config/design-tokens.jsonc`、`standards/dependencies.md`；优先调用内部生成器。
...
- 提交、推送、review：`checklists/pre-commit-review.md`，并按其条件加载检查模块、技术设计、项目生成和需求追溯检查。
- 文档或工作流：只读取直接相关文件。
