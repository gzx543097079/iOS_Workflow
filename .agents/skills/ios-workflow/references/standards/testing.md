...
- 新增或本次修改导致的 flaky test 阻止验收交付，仍可按 checkpoint 保存如实记录的阶段进度。既有不稳定测试只有在项目已批准隔离且记录负责人、跟踪项和到期时间时才能标记为已知问题。
...
- 测试结果中的 `✅` 只表示已取得与当前证据键匹配的成功结果；`❌` 表示失败或必需证据缺失；`➖` 表示需求未影响且说明原因。这些验证结果与保存操作门禁分开报告，不能因提交成功把测试失败或未知改成通过。
- 全部必需验收有成功证据、阻塞与未覆盖风险已处理，才可将需求标记 `done`；这不是保存阶段进度的前提。
- 提交检查只覆盖本次 diff 与声明完成的范围。用户要求保存未完成进度或跨设备交接时，提交及其明确授权的推送按 `operation=checkpoint` 检查，可保存如实记录的未完成/未验证状态和失败结果；范围、隐私、用户文件保护和记录完整性必须核实通过。验证失败或未知不自动阻止阶段保存；不得伪造通过，也不得据此合并为已验收交付或发布。普通提交和发布仍需相应范围的成功验证。
