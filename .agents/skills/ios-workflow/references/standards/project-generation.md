...
1. 由 `scripts/project_generation.py` 直接读取并校验 `assets/config/defaults.jsonc` 和 `assets/config/design-tokens.jsonc`，调用方不把完整配置输出到模型上下文；字段、版本、Bundle ID、本地化或技术组合非法时，在写入项目文件前停止。
